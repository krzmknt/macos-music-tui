...
// Used to achieve proper playlist/album context playback (avoiding AutoPlay mode)
...
/// Select a playlist in the sidebar using AppleScript (System Events)
...
fn click_play_button() -> Result<(), String> {
    ...
    let main_window = music_app
        ...
        .map_err(|_| "No main window - Music.app window must exist")?;
...
/// Play a playlist with proper context (not AutoPlay)
/// This uses a hybrid approach: AppleScript to select, Accessibility API to click Play
pub fn play_playlist_with_context(playlist_name: &str) -> Result<(), String> {
    // Ensure Music.app window exists
    let _ = Command::new("osascript")
        .arg("-e")
        .arg("tell application \"Music\" to reopen")
        .output();

    std::thread::sleep(std::time::Duration::from_millis(300));

    // Select the playlist in sidebar
...
/// Play an album with proper context (not AutoPlay)
/// First navigates to Albums view, then finds and plays the album
pub fn play_album_with_context(album_name: &str) -> Result<(), String> {
    ...
    // Navigate to Albums in sidebar first
    select_sidebar_item("Albums")?;
    std::thread::sleep(std::time::Duration::from_millis(300));

    // Search for the album using the search field or scroll to find it
    // For now, use AppleScript to reveal the album
